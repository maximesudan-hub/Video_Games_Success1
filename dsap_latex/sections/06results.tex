...
Hardware: Apple Silicon (M1) 8-core CPU (macOS). Software: Python 3.13 with numpy, pandas, scikit-learn, matplotlib. Protocol: random train/test split (80/20), random\_state = 42.
...
\centering
\includegraphics[width=0.9\linewidth]{regression_pred_vs_true.png}
...
\bottomrule
\end{tabular}
\captionof{table}{Regression performance on Log\_Sales (test set).}
...
\label{fig:feat_imp}
\end{figure}
Feature importance from Random Forest indicates that engagement variables (User Count, Critic Count) dominate, followed by platform and publisher effects. The ranking suggests that visibility (counts) carries more predictive signal than average scores, while genre and rating have smaller contributions.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.55\linewidth]{classification_roc_curve.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\linewidth]{clustering_pca.png}
...
\begin{minipage}{0.46\linewidth}
\centering
\includegraphics[width=0.9\linewidth]{clustering_profile_sales.png}
\caption{Mean Global Sales by cluster (millions of units).}
\label{fig:cluster_sales}
\end{minipage}\hfill
\begin{minipage}{0.46\linewidth}
\centering
\includegraphics[width=0.9\linewidth]{clustering_profile_critic_count.png}
